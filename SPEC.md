...
- **Unified shard-duration ladder per tier.** Each tier carries a `shards: Shard[]` ascending ladder (smallest ≥ `bin`; each divides the next). Drops the singular `tier.shard` + pyramid-wide `partials` + `partialKey` dichotomy that previously distinguished "canonical" from "partial" sub-shards — they're all just shards at different durations. Compactor writes at `shards[0]`; promotes up the ladder at each boundary; retention is per-(tier, shardDur). The planner uses a **cursor-aware-largest-first** walk: at each cursor position, try the largest `shardDur` at the output tier; fall to next smaller within tier on no-cover; fall to next finer tier on tier-exhausted (`reaggregate: true`). Fixes a walk-order pathology where the prior ascending-effective walk dropped prefix windows that smaller-cadence entries could have served. Watermark keys uniformly `${tier}@${shardDur}`; `earliestPerCadence` renamed to `earliestPerShard`. Storage layout becomes uniform `<tier>/<shard>/<period>`. Supersedes `partial-shards.md`, `per-cadence-earliest.md`, and `mixed-tier-tail-coverage.md`. See [`specs/done/unified-shard-ladder.md`](./specs/done/unified-shard-ladder.md).
