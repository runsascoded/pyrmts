...
- **Partial sub-shards via `ShardIndex` backend.** Each tier can opt into a pyramid-wide cadence ladder of partial sub-shards (`1h`, `1d`, etc.) that close the gap between the last sealed canonical shard and "now". `planQuery` walks the 2D `(tier, cadence)` watermark grid (within-tier and across-tier `min` propagation), emitting one segment per shard. Watermarks come from a pluggable `ShardIndex` — `D1ShardIndex` (CFW, atomic per-row upserts; recommended for concurrent cron writers) or `ManifestShardIndex` (JSON blob over any `Storage`). Both pass the same `assertShardIndexConformance` suite. See [`specs/done/partial-shards.md`](./specs/done/partial-shards.md).
- **Per-`(tier, cadence)` earliest watermarks (no propagation).** `PlanQueryInput.earliestPerCadence` gates individual `(tier, cadence)` entries in the watermark grid without propagating up the tier ladder — unlike `earliestWatermarks`, which clamps coarser tiers by their finer source's earliest. When a partial sub-shard's entire range is gated, the cursor doesn't advance past the entry, letting subsequent entries / coarser tiers serve the range. Driven by ctbk avail-v3's cascading `/1m` partials: forward-only coverage from the cron deploy date that must NOT poison `/2m+` canonical (full backfill). See [`specs/done/per-cadence-earliest.md`](./specs/done/per-cadence-earliest.md).
