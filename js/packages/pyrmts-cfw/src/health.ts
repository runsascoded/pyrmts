...
import {
  ...
  shardBuildableAt,
  type Pyramid,
  type PyramidCoverRung,
  type PyramidCoverSegment,
  type PyramidCoverStatus,
  type PyramidTierCoverStatus,
  type Storage,
} from 'pyrmts'
import type { D1Like } from './d1.js'

// Cover-status shapes moved to `pyrmts` core (`cover-status.ts`) so
// `pyrmts-react` can consume them worker-free; re-exported here for
// existing importers.
export type {
  PyramidCoverRung,
  PyramidCoverSegment,
