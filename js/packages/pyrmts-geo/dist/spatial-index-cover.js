...
export function minimalCover(index, include, system, opts = {}) {
    if (include.length === 0)
...
function buildTree(index, include, system, coarsestLevel) {
    ...
    while (byLevel.size > 0) {
        let active = 0;
        for (const b of byLevel.values())
            active += b.size;
        if (stopped + active === 1)
            break; // frontier collapsed to a single root
        ...
        for (const cellId of cells) {
            ...
            if (systemSet.has(parentCell)) {
                ...
                throw new Error(`minimalCover: system cells must be mutually disjoint; '${parentCell}' is an ancestor of '${leaf.cell}'`);
            }
...
function encodeInclude(node) {
    if (node.includeCount === 0)
        return [];
    if (node.excludeCount === 0)
        return [{ cell: node.cell, op: '+' }];
    // Mixed.
...
function encodeIncludePureUnion(node) {
    if (node.includeCount === 0)
        return [];
    if (node.excludeCount === 0)
        return [{ cell: node.cell, op: '+' }];
...
export function isCellInCover(index, cell, cover) {
...
//# sourceMappingURL=spatial-index-cover.js.map