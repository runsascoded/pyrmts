...
export function buildVocabGraph(
  ...
  for (const cell of cells) {
    if (!vocab.has(cell)) vocab.set(cell, { term: cell, isLeaf: false, children: [] })
  }

  // Nearest vocab member on `cell`'s ancestor chain (including itself).
  const containingVocab = (cell: string): string | null => {
    let cur = cell
    for (;;) {
      if (vocab.has(cur)) return cur
      if (index.cellLevel(cur) === 0) return null
  ...
  for (const [cell, node] of vocab) {
    ...
    if (anc === null) roots.push(node)
    else vocab.get(anc)!.children.push(node)
  }
  const seen = new Set<string>()
  for (const { key, cell } of leaves) {
    if (seen.has(key)) throw new Error(`buildVocabGraph: duplicate leaf key ${key}`)
    seen.add(key)
    const node: VocabNode = { term: key, isLeaf: true, children: [] }
...
export function vocabCover(
  ...
  const collectLeaves = (n: VocabNode): void => {
    if (n.isLeaf) leafTerms.add(n.term)
  ...
  if (unknown.length) {
    throw new Error(
      `vocabCover: ${unknown.length} wanted key(s) not in the vocab graph: `
      + unknown.slice(0, 5).join(', ') + (unknown.length > 5 ? ', …' : ''),
    )
  }

  const counts = new Map<VocabNode, { w: number, u: number }>()
  const count = (n: VocabNode): { w: number, u: number } => {
    let w = 0
    let u = 0
    if (n.isLeaf) {
      if (want.has(n.term)) w = 1
      else u = 1
    }
  ...
  const posMemo = new Map<VocabNode, Op[]>()
  const negMemo = new Map<VocabNode, Op[]>()

  const pos = (n: VocabNode): Op[] => {
    const hit = posMemo.get(n)
    if (hit !== undefined) return hit
    const { w, u } = counts.get(n)!
    let out: Op[]
    if (w === 0) out = []
    else if (u === 0) out = [{ term: n.term, op: '+' }]
    else {
      const explicit = n.children.flatMap(pos)
      if (opts.positiveOnly) out = explicit
      else {
        const viaParent: Op[] = [{ term: n.term, op: '+' }, ...n.children.flatMap(neg)]
  ...
  const neg = (n: VocabNode): Op[] => {
    const hit = negMemo.get(n)
    if (hit !== undefined) return hit
    const { w, u } = counts.get(n)!
    let out: Op[]
    if (u === 0) out = []
    else if (w === 0) out = [{ term: n.term, op: '-' }]
    else {
      const explicit = n.children.flatMap(neg)
      const viaParent: Op[] = [{ term: n.term, op: '-' }, ...n.children.flatMap(pos)]
      out = explicit.length <= viaParent.length ? explicit : viaParent
    }
    negMemo.set(n, out)
    return out
