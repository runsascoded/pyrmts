...
import { identities, keyStateCols } from './multiscan.js';
...
export function changesetFromRows(rows, schema) {
...
export function changesetToRows(cs, schema) {
    const { keyCols, stateCols } = keyStateCols(schema);
...
export function changesetBetween(rowsA, rowsB, schema) {
    const { keyCols, stateCols } = keyStateCols(schema);
    const ids = identities(schema);
    const id = stateCols.map(c => ids[c]);
    const a = new Map();
    const b = new Map();
    for (const r of rowsA)
        a.set(keyStr(r, keyCols), { keyRow: r, state: stateCols.map(c => r[c] ?? null) });
    for (const r of rowsB)
        b.set(keyStr(r, keyCols), { keyRow: r, state: stateCols.map(c => r[c] ?? null) });
    const out = new Map();
    for (const k of new Set([...a.keys(), ...b.keys()])) {
        const sa = a.get(k)?.state ?? id;
        const sb = b.get(k)?.state ?? id;
        if (!stateEq(sa, sb))
            out.set(k, { keyRow: (a.get(k) ?? b.get(k)).keyRow, sa, sb });
    }
    return out;
}
...
export function composeChangesets(left, right) {
    const out = new Map(left);
    for (const [k, { keyRow, sa: mid, sb }] of right) {
        const prev = out.get(k);
        if (prev) {
            if (stateEq(prev.sa, sb))
                out.delete(k);
            else
                out.set(k, { keyRow: prev.keyRow, sa: prev.sa, sb });
        }
        else {
...
export function alignedBlocks(i, j, levels = 0) {
    if (levels < 0)
        throw new Error(`alignedBlocks: levels must be ≥ 0, got ${levels}`);
    const out = [];
    let pos = i;
    while (pos < j) {
        let level = 0;
        while (level < levels && pos % (1 << (level + 1)) === 0 && pos + (1 << (level + 1)) <= j)
            level++;
        out.push([level, pos]);
        pos += 1 << level;
    }
...
export async function diffOverSpan(scans, schema, a, b, loadNode, levels = 0) {
    let i = scans.indexOf(a);
    let j = scans.indexOf(b);
    if (i < 0)
        throw new Error(`diffOverSpan: '${a}' not in the index`);
    if (j < 0)
        throw new Error(`diffOverSpan: '${b}' not in the index`);
    const reverse = i > j;
    if (reverse)
        [i, j] = [j, i];
    let result = new Map();
    ...
    if (reverse) {
        for (const [k, e] of result)
            result.set(k, { keyRow: e.keyRow, sa: e.sb, sb: e.sa });
    }
...
export async function readChangesetNode(bytes) {
...
export function parseDiffIndexManifest(bytes, dataset) {
...
//# sourceMappingURL=diffindex.js.map