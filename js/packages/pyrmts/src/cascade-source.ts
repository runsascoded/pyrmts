...
function approxMs(dur: string): number {
  const { count, unit } = parseDuration(dur)
  if (unit === 'mo') return count * 30 * DAY_MS
  if (unit === 'y') return count * 365 * DAY_MS
...
export function sourceTierFor(
  pyramid: Pick<Pyramid, 'tiers'>,
  tierName: string,
): Tier | null {
  const tiers = pyramid.tiers
  if (tierName === tiers[0]!.name) return null
  const tierIdx = tiers.findIndex((t) => t.name === tierName)
  if (tierIdx < 0) throw new Error(`sourceTierFor: no tier '${tierName}'`)
  ...
  for (const cand of tiers.slice(0, tierIdx)) {
    ...
    if (candMs >= targetMs || targetMs % candMs !== 0) continue
    if (candMs > bestMs) {
      best = cand
      bestMs = candMs
    }
...
export function shardBuildableAt(
  pyramid: Pick<Pyramid, 'tiers'>,
  tierName: string,
  periodEnd: Date,
): Date {
  const src = sourceTierFor(pyramid, tierName)
  if (src === null) return periodEnd
