...
// One repair request: shards overlapping `[start, end)` that were last
// built before `requestedAt` are stale.
...
// Wire format matches the Python impl + `specs/shard-invalidation.md`:
// `{start, end, requested_at}` (snake_case). Only the on-wire field
// name is snake_case; the TS `Invalidation` interface stays camelCase
// per JS convention.
interface InvalidationJson {
  start: number
  end: number
  requested_at: number
}
...
export function journalKey(pyramid: Pyramid): string {
  return pyramid.keyTemplate.split('{')[0]! + JOURNAL_BASENAME
}

function encode(invs: Invalidation[]): Uint8Array {
  const doc: InvalidationJson[] = invs.map(inv => ({
...
function decode(bytes: Uint8Array): Invalidation[] {
  const doc = JSON.parse(new TextDecoder().decode(bytes)) as InvalidationJson[]
  return doc.map(e => ({
...
function requireCas(storage: Storage): {
  getWithEtag: NonNullable<Storage['getWithEtag']>
  putIfMatch: NonNullable<Storage['putIfMatch']>
} {
  const { getWithEtag, putIfMatch } = storage
  if (!getWithEtag || !putIfMatch) {
    throw new NotSupported(
      'invalidation: storage backend must implement getWithEtag + putIfMatch',
    )
  }
...
export async function invalidate(
  ...
  if (!(start < end)) {
    throw new Error(
      `invalidate: empty interval [${start.toISOString()}, ${end.toISOString()})`,
    )
  }
  const { putIfMatch } = requireCas(storage)
  const entry: Invalidation = {
    start,
    end,
  ...
  for (let attempt = 0; attempt < CAS_ATTEMPTS; attempt++) {
    ...
    try {
      ...
      if (e instanceof EtagConflict) {
        if (attempt === CAS_ATTEMPTS - 1) throw e
        continue
      }
      throw e
    }
...
export function staleKeysFor(
  ...
  if (invalidations.length === 0) return new Set()
  const out = new Set<string>()
  for (const e of expected) {
    const mtime = mtimes.get(e.key)
    if (mtime === undefined || mtime === null) continue
...
export async function pruneSpent(
  ...
  storage: Storage,
  expected: ExpectedShard[],
  opts: { mtimes?: Map<string, Date | null> } = {},
  ...
  for (let attempt = 0; attempt < CAS_ATTEMPTS; attempt++) {
    const [invs, etag] = await loadInvalidations(pyramid, storage)
    if (invs.length === 0) return [0, 0]
    const keep = invs.filter(inv => staleKeysFor(expected, mtimes, [inv]).size > 0)
    if (keep.length === invs.length) return [0, invs.length]
    try {
...
export async function listExistingWithMtime(
  pyramid: Pyramid,
  storage: Storage,
