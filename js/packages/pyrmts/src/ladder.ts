...
export interface ValidatedTierLadder {
  tier: Tier
  // Ascending, same length as `tier.shards`. Each entry is the shard
  // duration in ms (fixed-width) or `null` (calendar-variable mo/y).
  // Fixed-width / calendar can't mix in a single ladder.
  shardsMs: Array<{ shard: Shard; ms: number | null }>
}
  ...
  for (let i = 0; i < tier.shards.length; i++) {
    ...
    if (shard !== '1run') {
      ...
      if (months !== null) {
        if (i === 0 && binMonths !== null) {
          if (months < binMonths) {
            throw new Error(
              `validateLadders: tier '${tier.name}' shards[0]='${shard}' ` +
              `is smaller than bin '${tier.bin}' (in months)`,
            )
          }
          if (months % binMonths !== 0) {
            throw new Error(
              `validateLadders: tier '${tier.name}' bin '${tier.bin}' does not ` +
        ...
        if (prevMonths !== null && months % prevMonths !== 0) {
          throw new Error(
            `validateLadders: tier '${tier.name}' shards[${i - 1}]='${tier.shards[i - 1]}' ` +
      ...
      if (prevNominal !== null && (months !== null || prevMonths !== null) && nominal <= prevNominal) {
        throw new Error(
          `validateLadders: tier '${tier.name}' shards not ascending ` +
    ...
    if (ms !== null) {
      if (binMs !== null && ms < binMs) {
        throw new Error(
          `validateLadders: tier '${tier.name}' shards[${i}]='${shard}' (${ms}ms) ` +
...
function shardMsOrNull(shard: Shard, tierName: string): number | null {
  if (shard === '1run') return null
  const parsed = parseDuration(shard)
  if (parsed.unit === 'mo' || parsed.unit === 'y') return null
  return fixedDurationMs(shard)
}

function binMsOrThrow(tier: Tier): number | null {
  const bin = tier.bin
  if (isStepBin(bin)) {
    return null  // step-axis bins don't measure in ms
  }
  const parsed = parseDuration(bin as Duration)
  if (parsed.unit === 'mo' || parsed.unit === 'y') return null
  return fixedDurationMs(bin as Duration)
}

function isStepBin(bin: string): boolean {
...
function monthsOrNull(dur: string): number | null {
  const parsed = parseDuration(dur as Duration)
  if (parsed.unit === 'mo') return parsed.count
  if (parsed.unit === 'y') return 12 * parsed.count
  return null
}
