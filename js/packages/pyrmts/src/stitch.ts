...
import { getMonoid, type Row } from './monoids.js'
...
export interface StitchInput {
  pyramid: Pyramid
  plan: QueryPlan
  // shardRows[i] holds the decoded rows for plan.segments[i].
  shardRows: Row[][]
}

export function stitch(input: StitchInput): Row[] {
  const { pyramid, plan, shardRows } = input
  if (shardRows.length !== plan.segments.length) {
    throw new Error(
      `stitch: shardRows length ${shardRows.length} ≠ segments ${plan.segments.length}`,
    )
  }

  const outputSpan = parseDuration(plan.outputBin as Duration)
  ...
  for (let i = 0; i < plan.segments.length; i++) {
    ...
    for (const row of rows) {
      const ts = row[binCol] as number
      if (typeof ts !== 'number') {
        throw new Error(
          `stitch: row missing numeric '${binCol}' column (got ${typeof ts})`,
        )
      }
...
function sortRows(rows: Row[], binCol: string, dimNames: string[]): Row[] {
  rows.sort((a, b) => {
    const ta = a[binCol] as number
    const tb = b[binCol] as number
