...
**Addendum (2026-09-07) — fill mode defers shards over an absent open tile.** `72f2552` only forgave the absence *after* the build: an uncapped `-f` on a tiled source still planned the trailing rungs over the day-in-progress as fillable, wrote them as 0-row shards, and every later fill then read them as built (ctbk: 28 empty `smg-v1` trailing shards, 3 inside `avail-v6`'s live tip, 12 per `rides-v5` anchor over the unpublished month). Now, before the window walk, `build_local(fill=True)` asks a tiled source for `tiles_for(from, to)`, LISTs the open ones (`period.end > to`), and moves every missing shard whose effective period overlaps an *absent* open tile from `fillable` to `deferred` — reported as `fill: N deferred (open-period source absent: <key> [<start>, <end>)): …`, counted in `BuildResult.deferred`, not an error. A later fill (after the tile lands) builds exactly those shards. Absent *closed* tiles are unchanged: their shards build and the guard fires as before (a late upstream is a real hole). `strict_open_periods=True` skips the deferral too. Tests: `test_raw_source.py::test_fill_defers_shards_over_absent_open_tile` (two-fill round trip) and `::test_fill_strict_open_periods_builds_over_absent_open_tile`.
