...
- **Phase 2b (remaining).** The **read-path serve integration** (over-time + diff, `series_for`/`diff_scans` behind the reader) is the consumer's — cw-s3 owns the DuckDB producer + CFW reader (`specs/obs-axis-indexing.md` in mgu). The remaining pyrmts-side open item is the **config surface** for an opt-in MS layout (leaning driver-only + storage-key convention, no new `Pyramid` block — see Open questions).
