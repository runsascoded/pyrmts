...
- **Phase 2c (scan-location manifest + safe drop).** ✓ **Done.** The routing overlay: `pyrmts_engine.multiscan_index` (`MultiScanRecord`, `MemMultiScanIndex` / `StorageJsonlMultiScanIndex`, `resolve_scan`); the driver records a row per consolidated tile (`consolidate_range(ms_index=, dataset=)`) and `drop_consolidated_scans` verifies-then-deletes the individuals; the `multiscan consolidate` CLI gains `--dataset`/`--index`/`--drop`. TS reader-side twin in `js/packages/pyrmts/multiscan.ts`: `MultiScanIndexEntry`, `parseMultiScanIndex`, `resolveScan` (same JSONL row shape). D1 (`pyramid_multiscans`) is a follow-up; the JSONL shape is D1-ready.
- **Phase 2 remaining open item:** the **config surface** for an opt-in MS layout (leaning driver-only + storage-key convention, no new `Pyramid` block — see Open questions; a block can be added later without reshaping the above).
