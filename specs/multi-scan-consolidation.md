...
4. **Decide a/b**, then build the read path + finalize the CLI. ✓ **(b) decided** — Phase 2 (read path) unblocked.
...
- **Phase 2 (a/b confirmed on real data ✓ — now unblocked).** Read path (point / diff / range) + `pyrmts_engine consolidate`/`extract` storage driver + config surface for the opt-in MS layout. The real-scan run adds two Phase-2 requirements: (i) the driver must **stream / chunk** (or push the consolidation into DuckDB) rather than build a full per-scan key-dict — 6 M+ keys × tens of scans OOMs the naive one-process encoder; (ii) accept **snapshot indices** by mapping them to a constant-`binCol` pyramid (document in the driver contract).
