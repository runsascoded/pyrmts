...
- **Phase 2c (scan-location manifest + safe drop).** The routing overlay (`pyramid_multiscans`): `MultiScanRecord` + a JSONL/`Storage` index (Python driver-side write + TS `resolveScan` reader-side route), the driver recording a row per consolidated tile, and the verify-then-drop safety helper. See "Scan-location manifest" above.
